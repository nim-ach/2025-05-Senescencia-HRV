...
get_differences <- function(model, fun_data = NULL, grid = NULL) {
  if (is.null(fun_data)) {
    fun_data <- function(i) { return(i) }
  }
  if (is.null(grid)) {
    grid <- data.frame(id = NA, sex = NA,
                       time = c("Pre", "Peri", "Post"),
                       age = 0, bp_diastolic = 0,
                       imc = 0, bp_systolic = 0,
                       frail_total = 0, frail_above_0 = NA,
                       velocity_tm6 = 0)
  }
  tidybayes::epred_draws(model,
                         newdata = grid,
                         allow_new_levels = TRUE) |>
    dplyr::group_by(.category) |>
    tidybayes::compare_levels(.epred, by = time,
                              comparison = list(c("Peri","Pre"),
                                                c("Post","Peri"),
                                                c("Post","Pre"))) |>
    fun_data()
}

plot_hrv_over_time <- function(model_data) {
  
  ggplot(model_data, aes(time, .epred)) +
    facet_wrap(~ .category, nrow = 3, ncol = 3, scales = "free_y") +
    annotate(geom = "ribbon", x = c(-Inf, Inf), ymin = -0.8, ymax = 0.8,
             fill = "gray95") +
    annotate(geom = "ribbon", x = c(-Inf, Inf), ymin = -0.5, ymax = 0.5,
             fill = "gray90") +
    annotate(geom = "ribbon", x = c(-Inf, Inf), ymin = -0.1, ymax = 0.1,
             fill = "gray80") +
    tidybayes::stat_halfeye(aes(fill = after_stat(ifelse(abs(y) > 0.1, "Significant", "Non-Significant")))) +
    geom_hline(yintercept = 0, linetype = 2, linewidth = 1/3) +
    labs(fill = "ROPE", x = NULL, y = "Standardized Mean Difference (SMD)") +
    scale_fill_manual(values = c("gray20", "orange2")) +
    theme(legend.position = "top",
          axis.line.x.bottom = element_line(),
          axis.text.x.bottom = element_text(angle = 45, vjust = .9, hjust = .9))
}
